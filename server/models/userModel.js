...
exports.crearUsuarioQuery = async (
  carnet,
  nombres,
  apellidos,
  correo,
  ...
  const results = await sequelize.query('CALL GetKeyUser(:carnet);', {
    replacements: { carnet: carnet },
    type: sequelize.QueryTypes.SELECT,
  });
...
exports.updatePasswordQuery = async (correo, carnet, contrasena) => {
  const results = await sequelize.query(
    'UPDATE usuario SET contrasena = :contrasena WHERE carnet = :carnet AND correo = :correo',
    {
...
exports.getAllUsersQuery = async () => {
  const [results, metadata] = await sequelize.query('CALL GetAllUsers();', {
    type: sequelize.QueryTypes.SELECT,
  });
